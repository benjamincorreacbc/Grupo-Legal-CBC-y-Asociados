...
const Auth = (() => {
  ...
  async function ensure() {
    ...
    if (error) {
      console.warn('No se pudo validar la sesión remota. Se usará la información local si existe.', error);
      if (cached) {
        return cached;
      }
      logout();
...
const LocalDataService = (() => {
  ...
  function hydrate(nextState) {
    try {
      cache = nextState ? Utils.clone(nextState) : createDefaultState();
    } catch (error) {
      console.warn('No se pudo clonar el estado remoto. Se usará el predeterminado.', error);
  ...
  return { load, getState, dispatch, hydrate };
})();
...
const DataService = (() => {
  let mode = USE_REMOTE_API ? 'remote' : 'local';
  let active = mode === 'remote' ? RemoteDataService : LocalDataService;
  let warnedFallback = false;

  window.__GLCBC_ACTIVE_MODE__ = mode;

  function setMode(nextMode) {
    if (nextMode === mode) return;
    if (nextMode === 'remote' && !USE_REMOTE_API) {
      console.warn('No hay API remota configurada. Se mantiene el modo local.');
      return;
    }
    mode = nextMode === 'remote' ? 'remote' : 'local';
    active = mode === 'remote' ? RemoteDataService : LocalDataService;
    window.__GLCBC_ACTIVE_MODE__ = mode;
    if (mode === 'remote') {
  ...
  function syncLocal(result) {
    if (mode === 'remote' && result && typeof LocalDataService?.hydrate === 'function') {
      LocalDataService.hydrate(result);
    }
  }

  function notifyFallback(error, context) {
    console.warn(`Fallo ${context} remoto. Se usará el modo local.`, error);
    if (!warnedFallback) {
      UI.showToast('No se pudo conectar al servidor. Trabajarás en modo sin conexión.', 'warning', 6000);
      warnedFallback = true;
    }
    setMode('local');
  }
  ...
  async function load() {
    try {
      const result = await active.load();
      syncLocal(result);
      return result;
    } catch (error) {
      if (mode === 'remote') {
        notifyFallback(error, 'al cargar el estado');
        return active.load();
      }
      throw error;
    }
  ...
  async function dispatch(action, payload) {
    try {
      const result = await active.dispatch(action, payload);
      syncLocal(result);
      return result;
    } catch (error) {
      if (mode === 'remote') {
        notifyFallback(error, `al ejecutar ${action}`);
        return active.dispatch(action, payload);
      }
      throw error;
    }
  }
  ...
  return { load, getState, dispatch, isRemote, isLocal, useRemote, mode: modeLabel };
})();
...
window.requestAccount = async function requestAccount() {
  try {
    ...
    const remoteActive = typeof DataService?.isRemote === 'function' ? DataService.isRemote() : USE_REMOTE_API;
    if (remoteActive && window.Http) {
      try {
    ...
    } else if (DataService && typeof DataService.dispatch === 'function') {
