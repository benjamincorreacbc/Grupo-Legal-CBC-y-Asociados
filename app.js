...
const Portal = (() => {
  ...
  async function handleAddFee(event) {
    ...
    if (!activeCaseId) return;
    const data = new FormData(event.currentTarget);
    const amount = Number(data.get('feeAmount'));
    if (!amount || amount <= 0) {
      UI.showToast('Monto inválido.', 'error');
      return;
    }
    const withIVA = data.get('feeIVA') === 'on';
    const currency = data.get('feeCurrency') || 'CLP';
    const splitRaw = (data.get('feeSplit') || '')
      .split(',')
      .map((entry) => entry.trim())
      .filter(Boolean)
      .map((entry) => {
        const [userId, percent] = entry.split(':');
        return { userId: userId?.trim(), percent: Number(percent) || 0 };
      });
    try {
      await DataService.dispatch('fees.create', {
        caseId: activeCaseId,
        concept: data.get('feeConcept') || 'Honorario',
        amount,
        iva: withIVA ? state.parameters.iva : 0,
        currency,
        dueDate: data.get('feeDue'),
        splits: splitRaw,
      });
      state = DataService.getState();
  ...
  // --------------------------- Chat -------------------------------------
  function renderChat() {
    const threadsNode = document.getElementById('chatThreads');
  ...
  function renderOfficeBookings() {
    const list = document.getElementById('officeBookingsList');
    if (!list) return;
    ...
    const officesSelect = document.getElementById('reservationOffice');
    if (officesSelect) {
  ...
  async function handleAddReservation(event) {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const officeId = data.get('reservationOffice');
    const start = data.get('reservationStart');
    const end = data.get('reservationEnd');
    const reason = data.get('reservationReason');
    if (!officeId || !start || !end || !reason) {
      UI.showToast('Completa oficina, fechas y motivo.', 'error');
      return;
    }
    try {
  ...
  async function handleAddMeeting(event) {
    ...
    const start = data.get('meetingStart');
    const end = data.get('meetingEnd');
    const participants = (data.get('meetingParticipants') || '')
      .split(',')
      .map((id) => id.trim())
      .filter(Boolean);
    if (!title || !start || !end) {
      UI.showToast('Completa título y horario.', 'error');
      return;
    }
    try {
      await DataService.dispatch('meetings.create', {
        ...
        caseId: data.get('meetingCase') || null,
        participantIds: participants,
      });
      state = DataService.getState();
